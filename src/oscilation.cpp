...
void seta_PMNS(complex<double> U[][3], bool twogen){
	...
	double deltaCP = 195 * M_PI / 180; /*195*/
...
int Hamiltoniano_sol(double x, const double nu_f[], double f[], void *parametros){
	...
	/*mass mixing parameters*/
	double Dmq21 = 9e-5;//7.42e-5;
	double Dmq31 = 2.51e-3;
	
	/*************/
	/*HAMILTONIAN*/
	/*************/
	double H[6][6];
	complex<double> matter_H[3][3];
	complex<double> flavor_H[3][3];
	complex<double> U[3][3];
	complex<double> Udagger[3][3];
	seta_PMNS(U, false);
	for(int i = 0; i < 3; i++)
	...
	double cte = 1.26693276;
	cte *= 4;
	double cte3 = 0.0003867928440314046;
	
	/*CONTRIBUTIONS THAT ARE DIAGONAL IN THE MASS BASE*/
	matter_H[1][1] += (cte/E)*Dmq21;
	matter_H[2][2] += (cte/E)*Dmq31;

	/*CONTRIBUTIONS THAT ARE DIAGONAL IN THE FLAVOR BASE*/
	if(x < Rsun){
		//flavor_H[0][0] += cte3*245*exp(-(10.54*x)/Rsun);
		flavor_H[0][0] += cte3*pow(10,electron_density_sun_NA(x/Rsun));
	}

	/*PUTTING THE matter_H CONTRIBUTIONS INSIDE flavor_H*/
	for(int i = 0; i < 3; i++)
		for(int j = 0; j < 3; j++)
			for(int k = 0; k < 3; k++)
				for(int l = 0; l < 3; l++)
					flavor_H[i][j] += U[i][k]*matter_H[k][l]*Udagger[l][j];


	/*setting the six dimensional real hamiltonian*/
	for(int i = 0; i < 3; i++){
		for(int j = 0; j < 3; j++){
			H[i][j+3] = real(flavor_H[i][j]);
			H[i+3][j] = -real(flavor_H[i][j]);
			H[i][j] = imag(flavor_H[i][j]);
			H[i+3][j+3] = imag(flavor_H[i][j]);
		}
	}
...
double survival_prob_sun(double E_nu, double x0, double L, double step_in_km, void *param, bool print_evolution, string output_file){
	/*numeric integration parameters*/
	const int dim = 6;
	const double erro_abs = 1e-6;
	const double erro_rel = 1e-6;
	int status;
	...
	/*REAL state vector in the flavor base, first (last) 3 entries are the real (imaginary) parts*/
	/*already with initial condition*/
	double nu_f[6] = {1, 0, 0, 0, 0, 0};
	...
	sistema.function = Hamiltoniano_sol;
	sistema.dimension = dim;
	sistema.params = parametros;

	gsl_odeiv2_driver *driver = gsl_odeiv2_driver_alloc_y_new(&sistema, gsl_odeiv2_step_rkf45, h, erro_abs, erro_rel);

	double check_norm, prob;
	...
	ofstream file;
	file.open(outdir + output_file);

	/*numeric integration*/
	for(x_atual = x_ini; x_atual < x_fim; x_atual += step_in_km){
		if(print_evolution){
			/*calculating the survival prob*/
			prob = nu_f[0]*nu_f[0] + nu_f[3]*nu_f[3];
			file << x_proximo << " " << prob << endl;
		}

		x_proximo = x_atual + step_in_km;
		status = gsl_odeiv2_driver_apply(driver, &x_atual, x_proximo, nu_f);
		if(status != GSL_SUCCESS){
			cout << "ERRO!!!" << endl;
			break;
		}

 		/*since the time evolution is unitary, the vector nu_m must always belong to S^3, so we correct this at each step*/
		check_norm = 0;
		for(int i = 0; i < 6; i++)
			check_norm += nu_f[i]*nu_f[i];
		for(int i = 0; i < 6; i++)
			nu_f[i] *= (1/check_norm);
 	}
 	...
 	/*now taking an average*/
 	double resultado = 0;
 	int N_pontos = 10000;

 	for(int i = 0; i < N_pontos; i++, x_atual += step_in_km*5){
 		/*calculating the survival prob*/
 		prob = nu_f[0]*nu_f[0] + nu_f[3]*nu_f[3];
 		//cout << prob << endl;
		resultado += prob/N_pontos;
...
double survivalProb_3genSun_Ad(double E, double Rfrac){
	...
	/*mixing angles*/
	double theta = 33.44 * M_PI / 180; /*theta12*/
	double theta13 = 8.57 * M_PI / 180; /*theta13*/
	double si2 = sin(2*theta);
	double co2 = cos(2*theta);

	/*mass mixing parameter*/
	double Dmq = 4.8e-5;//7.42e-5; /*Dmq21*/
